...
for page in range(1, 6):  # testing first 5 pages; adjust range as needed
    ...
    for url in statements:
        ...
        content = statement_soup.get_text().lower()  # converts to lowercase
        if any(kw in content for kw in keywords):
            matched_statements.append(url)

with open('matched_jewish_statements.txt', 'w') as f:
    for url in matched_statements:
        f.write(url + "\n")

print("Matching URLs saved to matched_jewish_statements.txt")
